...
function EditCustomer() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/customers/${uuid}`);
        const fetchedData = response.data;
        

        fetchedData.associatedSuppliers = Array.isArray(fetchedData.associatedSuppliers)
       ...
       if (fetchedData.contactDetails && Array.isArray(fetchedData.contactDetails)) {
        setContacts(fetchedData.contactDetails); // Set contacts in state
      }
       if (fetchedData.generalDetails && Array.isArray(fetchedData.generalDetails)) {
        setgeneralDetalis(fetchedData.generalDetails); // Set contacts in state
      }


        // Set SAP codes data
        if (fetchedData.sapCodesDetails && Array.isArray(fetchedData.sapCodesDetails)) {
...
const handleUpdate=async(e)=>{
  ...
  try {
    ...
    data.licenseValidTill = new Date(data.licenseValidTill); // Example for date handling

    // Send the updated data to the backend
    const response = await axios.put('http://localhost:3000/api/customers/updateCustomerDetails', data);
...
const removeContact = (index) => {
  ...
  const handlecontactupdate = async (e) => {
    ...
    const updatedContacts = data.contactDetails;
  
    // Access the customerId from the first object in the array
    const customerId = updatedContacts.length > 0 ? updatedContacts[0].customerId : null;
    ...
    try {
      ...
      const response = await axios.put(
        "http://localhost:3000/api/customers/updateCustomerContactDetails",
        payload
      );
      ...
      alert("An error occurred while updating contacts. Please try again later.");
    }
  };
  ...
  const handleUpdateGeneralDetalis = async (e) => {
    ...
    const dataToSend = generalDetalis.length > 0 ? generalDetalis[0] : generalDetalis; // Adjust this if needed
  
    // Log the data to ensure it's structured correctly
    console.log("Data to be sent to backend:", dataToSend);
  
    const API_URL = "http://localhost:3000/api/customers/updateCustomerGeneralDetails";
  ...
  const [SAPData, setSAPData] = useState([{ supplier: "", product: "", SAPCode: "" }]);
  // Handle change for contact details
  ...
  // Add a new contact detail
  const addSAP = () => {
    setSAP([...SAP, { supplierId: "",supplier: "", product: "", sapcode: "" }]);
  };
  // Remove a contact detail
  ...
  const handleSAPSave = async (e) => {
    ...
    const virtualAccountPayloads = SAP.map((item) => ({
      ...
      customerName: data.companyName, // Correct customer name
    ...
    try {
      ...
      const sapCodesResponse = await fetch("http://localhost:3000/api/customers/customerSAPCodesDetails", {
        method: "POST",
        headers: {
      ...
      const virtualAccountPromises = virtualAccountPayloads.map((accountPayload) =>
      ...
      alert("An error occurred while saving the data. Please check the console for details.");
    }
  };
  ...
  const handleDelete = async (SAPDataId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:3000/api/customers/deleteCustomerSAPCode`,
      ...
      if (response.status === 200) {
        ...
        setSAPData((prevList) => prevList.filter((sap) => sap.id !== SAPDataId));
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e)=>setData({...data,primaryPhoneNumber:e.target.value})}
                    />
                  </Grid>
                  {/* 4 */}
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e)=>setData({...data,secondaryPhoneNumber:e.target.value})}
                    />
                  </Grid>
                  {/* 5 */}
                  <Grid item xs={12} sm={6}>
  ...
  >
    {data.associatedSuppliers && data.associatedSuppliers.length > 0 ? (
      data.associatedSuppliers.map((supplier, index) => (
        <MenuItem key={index} value={supplier}>
          <Checkbox
            checked={
              data.associatedSuppliers.indexOf(supplier) > -1
            }
          />
      ...
      <MenuItem disabled>No Team Available</MenuItem>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth>
                      ...
                      <Select label="Type of Company" name="typeOfCompany" required
                       value={data.typeOfCompany || ""}
                       onChange={(e)=>setData({...data,typeOfCompany:e.target.value})}
                      >
    ...
    {data.ourCompanies && data.ourCompanies.length > 0 ? (
      data.ourCompanies.map((company, index) => (
        <MenuItem key={index} value={company}>
          <Checkbox checked={data.ourCompanies.indexOf(company) > -1} />
      ...
      <MenuItem disabled>No Companies Available</MenuItem>
                  ...
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e)=>setData({...data,tanNumber:e.target.value})}
                    />
                  </Grid>
                  {/* 17 */}
                  <Grid item xs={12} sm={6}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e)=>setData({...data,licenseValidTill:e.target.value})}
                    />
                  </Grid>
                  {/* 20 */}
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e)=>setData({...data,licenseCapacity :e.target.value})}
                    />
                  </Grid>
                  {/* 21 */}
                  <Grid item xs={12} sm={6}>
  ...
  <Select
    ...
    required
  >
    <MenuItem value={data.transporter}>{data.transporter}</MenuItem>
  </Select>
</FormControl>
                   ...
                   <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e)=>setData({...data,competitorSupplier:e.target.value})}
                    />
                  </Grid>
                </Grid>
                {/* Action Buttons */}
                    ...
                    >
                      ...
                      >
                        ...
                        <TextField
                          ...
                          onChange={(e) =>
                            handleContactChange(index, "commentRemark", e.target.value)
                          }
                          sx={{ flex: 1 }}
                        />
                      </Box>
              ...
              {activeTab === 2 && (
                  ...
                  {generalDetalis.map((details, index) => (
                    <Box key={index}>
                      ...
                      <Typography className="fs-4" sx={{ marginBottom: "20px", fontWeight: "500" }}>
                        General Details
                      </Typography>
                      <Grid container spacing={2}>
                        {/* Product Segment */}
                      ...
                      <Typography className="fs-4" sx={{ margin: "30px 0 20px", fontWeight: "500" }}>
                        Stock Details
                      </Typography>
                      <Grid container spacing={2}>
                        <Grid item xs={12} sm={6}>
...
<FormControl sx={{ flex: 1 }}>
  ...
  <Select
    ...
    onChange={(e) => handleSAPChange(index, "supplier", e.target.value)} // Pass supplierId on change
  >
...
<FormControl sx={{ flex: 1 }}>
  ...
  <Select
    ...
    onChange={(e) => handleSAPChange(index, "product", e.target.value)}
  >
...
<TextField
  ...
  <Accordion key={index}>
    ...
    <AccordionSummary
      ...
      sx={{ backgroundColor: "#f0f5fc", borderRadius: "4px 4px 0 0" }}
    >
      <Box
        sx={{
          display: "flex",
      ...
      >
        <Typography>
          <strong>Supplier:</strong> {detail.supplierName || "N/A"}
        </Typography>{" "}
        |
        <Typography>
          <strong>Product:</strong> {detail.productName || "N/A"}
        </Typography>{" "}
        |
        <Typography>
