...
const prisma = new PrismaClient();
  ...
  exports.createCustomerSAPCodesDetails = async (req, res) => {
    try {
      ...
      // Validate each SAP code entry
      for (const detail of sapCodesDetails) {
        if (!detail.supplierName || !detail.productName) {
          return res.status(400).json({
            error: 'Each SAP code entry must include supplierName and productName.',
          });
        }
      }
      ...
      const newSAPCodes = await prisma.customerSAPCodesDetails.createMany({
        data: sapCodesDetails.map((detail) => ({
          ...
          supplierName: detail.supplierName,
          productName: detail.productName,
          sapCode: detail.sapCode || null, // Allow sapCode to be null
        })),
      });
...
exports.createVirtualAccount = async (req, res) => {
  try {
    ...
    // Validate required fields
    if (!customerId || !customerName || !supplierId) {
      return res.status(400).json({
        error: 'customerId, customerName, and supplierId are required.',
      });
    }
